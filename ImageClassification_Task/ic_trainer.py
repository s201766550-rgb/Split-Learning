...
class ICTrainer:
    ...
    def merge_model_weights(self, epoch):
        ...
        for c_id, sc_client in self.sc_clients.items():
            try:
                center_back_sd = copy.deepcopy(sc_client.center_back_model.state_dict())
                transported_center_back_sd = self._transport_state_dict_for_comm(center_back_sd)
                self._add_comm_bytes('comm/fl/upload_bytes_epoch', self._state_dict_comm_bytes(transported_center_back_sd))
                restored_center_back_sd = self._restore_state_dict_from_comm(transported_center_back_sd, center_back_sd)
                params.append(restored_center_back_sd)
        ...
        try:
            ...
            transported_w_glob = self._transport_state_dict_for_comm(w_glob)
            self._add_comm_bytes(
                'comm/fl/download_bytes_epoch',
                self._state_dict_comm_bytes(transported_w_glob) * len(self.sc_clients)
            )
            for c_id, sc_client in self.sc_clients.items():
                local_ref = sc_client.center_back_model.state_dict()
                restored_w_glob = self._restore_state_dict_from_comm(transported_w_glob, local_ref)
                sc_client.center_back_model.load_state_dict(restored_w_glob)
        ...
        if True:
            ...
            for c_id, client in self.clients.items():
                try:
                    back_sd = copy.deepcopy(client.back_model.state_dict())
                    transported_back_sd = self._transport_state_dict_for_comm(back_sd)
                    self._add_comm_bytes('comm/fl/upload_bytes_epoch', self._state_dict_comm_bytes(transported_back_sd))
                    restored_back_sd = self._restore_state_dict_from_comm(transported_back_sd, back_sd)
                    params.append(restored_back_sd)
            ...
            try:
                ...
                transported_w_glob_cb = self._transport_state_dict_for_comm(w_glob_cb)
                self._add_comm_bytes(
                    'comm/fl/download_bytes_epoch',
                    self._state_dict_comm_bytes(transported_w_glob_cb) * len(self.clients)
                )
                for c_id, client in self.clients.items():
                    local_ref = client.back_model.state_dict()
                    restored_w_glob_cb = self._restore_state_dict_from_comm(transported_w_glob_cb, local_ref)
                    client.back_model.load_state_dict(restored_w_glob_cb)
            ...
            for c_id, client in tqdm(self.clients.items()):
                for it in tqdm(range(client.num_iterations * self.args.kv_factor), desc="client front"):
                    ...
                    if self.args.mixup:
                        ...
                        self.sc_clients[c_id].remote_activations1 = self._transport_client_to_server(
                            mixed_acts,
                            requires_grad=True,
                        )
                    else:
                        self.sc_clients[c_id].remote_activations1 = self._transport_client_to_server(
                            client.remote_activations1,
                            requires_grad=client.remote_activations1.requires_grad,
                        )

                    self._add_comm_bytes(
                        'comm/sl/c2s_activation_bytes_epoch',
                        self._tensor_comm_bytes(self.sc_clients[c_id].remote_activations1, fp16_on_wire=self.args.FP16)
                    )
    ...
    def _transport_client_to_server(self, activations, requires_grad):
        if self.args.FP16:
            transported = activations.detach().half().float().detach()
        else:
            transported = activations.detach()
    ...
    def _transport_state_dict_for_comm(self, state_dict):
        transported = {}
        for key, value in state_dict.items():
            if torch.is_tensor(value):
                if self.args.FP16 and torch.is_floating_point(value):
                    transported[key] = value.detach().half().cpu()
                else:
                    transported[key] = value.detach().cpu()
            else:
                transported[key] = value
        return transported

    def _restore_state_dict_from_comm(self, transported_state_dict, reference_state_dict):
        restored = {}
        for key, value in transported_state_dict.items():
            if torch.is_tensor(value):
                ref_value = reference_state_dict[key]
                restored[key] = value.to(device=ref_value.device, dtype=ref_value.dtype)
            else:
                restored[key] = value
        return restored

    def _tensor_comm_bytes(self, tensor, fp16_on_wire=False):
        if tensor is None:
            return 0
        if fp16_on_wire:
    ...
    def _state_dict_comm_bytes(self, state_dict):
        total = 0
    ...
    def _reset_comm_epoch_counters(self):
        self.comm_epoch_bytes = {
            'comm/sl/c2s_activation_bytes_epoch': 0,
            'comm/sl/s2c_activation_bytes_epoch': 0,
            'comm/sl/c2s_grad_bytes_epoch': 0,
            'comm/fl/upload_bytes_epoch': 0,
            'comm/fl/download_bytes_epoch': 0,
        }
    ...
    def _log_comm_epoch(self, epoch):
        total = (
            ...
            + self.comm_epoch_bytes['comm/sl/s2c_activation_bytes_epoch']
            + self.comm_epoch_bytes['comm/sl/c2s_grad_bytes_epoch']
    ...
    def train_one_epoch(self,epoch):
        ...
        for client_id, client in tqdm(self.clients.items()):
                ...
                for iteration in tqdm(range(client.num_iterations),desc="Generalization Phase Training"):
                    ...
                    self._add_comm_bytes(
                        'comm/sl/s2c_activation_bytes_epoch',
                        self._tensor_comm_bytes(client.remote_activations2, fp16_on_wire=self.args.FP16)
                    )
                    ...
                    wandb.log({'generalization/train step loss': client.loss.item()})
                    
                    client.loss.backward()
                    self._add_comm_bytes(
                        'comm/sl/c2s_grad_bytes_epoch',
                        self._tensor_comm_bytes(client.remote_activations2.grad, fp16_on_wire=self.args.FP16)
                    )
    ...
    def train_one_epoch_personalise(self,epoch):
        ...
        for client_id, client in tqdm(self.clients.items()):
                ...
                for iteration in tqdm(range(client.num_iterations),desc="Personlaisation Phase Training"):
                    ...
                    client.mixup_lam = None
                    client.mixup_targets_b = None
                    wandb.log({'personalize/train step loss': client.loss.item()})
                    client.step_back()
                    client.zero_grad_back()
                    #client.loss.backward()
                    f1=client.calculate_train_metric()
    ...
    def test_one_epoch_personalise(self,epoch):
        ...
        for client_id, client in tqdm(self.clients.items()):
                ...
                for iteration in tqdm(range(client.num_test_iterations),desc="Personalised Validation"):
                    ...
                    wandb.log({'personalize/Validation step loss': client.loss.item()})
    ...
    def test_one_epoch(self,epoch):
        ...
        for client_id, client in tqdm(self.clients.items()):
                ...
                for iteration in tqdm(range(client.num_test_iterations),desc="Validation"):
                    ...
                    client.forward_back()
                    client.calculate_loss(mode='test')
                    wandb.log({'generalization/Validation step loss': client.loss.item()})
                    f1=client.calculate_test_metric()
                    client.test_f1[-1] += f1 
                   #  #print("validation f1 per iteration: ",iteration,f1)
    ...
    def inference_new(self,):
        ...
        for idx, (c_id, client) in enumerate(self.clients.items()):
            ...
            general_back_sd = torch.load(self.save_dir / f'client_{c_id}_{self.args.model}_back.pth', map_location=self.device)
            self.clients[c_id].back_model.load_state_dict(general_back_sd)
            ...
            personal_back_sd = torch.load(self.save_dir / f'client_{c_id}_{self.args.model}_back_per.pth', map_location=self.device)
            self.clients[c_id].back_model.load_state_dict(personal_back_sd)
           ...
           #  #print("Shapes - preds:", preds.shape, "targets:", targets.shape)
            ...
            wandb.log({
                'hybrid_inference/cfm': wandb.plot.confusion_matrix(
                    preds=preds,
                    y_true=targets,
                    class_names=[f'{i}' for i in range(10)]
                )
    ...
    def fit(self,):
       ...
       #  # print('freeing some GPU...')
        ...
        self._reset_comm_epoch_counters()
